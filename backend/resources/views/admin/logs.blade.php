...
<div class="container">
    ...
    <div class="stats-grid">
        ...
        <div class="stat-card">
            ...
            <div class="stat-info">
                <h3>{{ $acciones->count() }}</h3>
                <p>Tipos de acciones</p>
            </div>
        </div>
    </div>

    {{-- BREAKDOWN POR ACCION --}}
    @if($acciones->isNotEmpty())
    <p class="section-title">Desglose por acción</p>
    <div class="breakdown-grid">
        @foreach($acciones as $ac)
        <a class="breakdown-card" href="{{ request()->fullUrlWithQuery(['accion' => $ac->accion]) }}">
            <div class="count">{{ $ac->total }}</div>
            <div class="label">{{ $ac->accion }}</div>
        </a>
        @endforeach
    </div>
    @endif
    ...
    <form method="GET" action="">
        <div class="filters-bar">
            <div class="filter-group">
                ...
                <input type="text" name="buscar" value="{{ request('buscar') }}" placeholder="Nombre, email, descripción...">
            </div>
            <div class="filter-group" style="max-width: 160px;">
                ...
                <select name="accion">
                    ...
                    @foreach($acciones as $ac)
                    <option value="{{ $ac->accion }}" @selected(request('accion') === $ac->accion)>{{ $ac->accion }}</option>
                    @endforeach
                </select>
            </div>
            <div class="filter-group" style="max-width: 140px;">
                ...
                <select name="rol">
                    ...
                    <option value="TEACHER" @selected(request('rol') === 'TEACHER')>TEACHER</option>
                    <option value="STUDENT" @selected(request('rol') === 'STUDENT')>STUDENT</option>
                </select>
            </div>
            <button type="submit" class="btn-filter">Filtrar</button>
            <a href="{{ route('admin.logs') }}" class="btn-reset">Limpiar</a>
        </div>
    </form>
    ...
    <div class="table-card">
        <div class="table-header">
            ...
            <span class="result-count">{{ $logs->count() }} resultados</span>
        </div>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="user-cell">
                                <strong>{{ $log->nombre_usuario ?? 'Sistema' }}</strong>
                                <small>{{ $log->correo_usuario ?? '—' }}</small>
                            </div>
                        </td>
                        <td>
                            <span class="{{ $rolClass }}" style="font-size: 0.8rem; font-weight: 600;">
                                {{ $log->rol ?? '—' }}
                            </span>
                        </td>
                        <td>
                        ...
                        <td>
                            <span class="desc-text" title="{{ $log->descripcion }}">
                                {{ $log->descripcion ?? '—' }}
                            </span>
                        </td>
                        <td>
